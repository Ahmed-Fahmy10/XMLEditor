...
#define COOKIE_XML_COMMENT		0x0020
...
DWORD CSampleView::ParseLine(DWORD dwCookie, int nLineIndex, TEXTBLOCK *pBuf, int &nActualItems)
{
	...
	if (nLength <= 0)
		return dwCookie & (COOKIE_EXT_COMMENT | COOKIE_XML_COMMENT);

	LPCTSTR pszChars    = GetLineChars(nLineIndex);
	BOOL bFirstChar     = (dwCookie & ~(COOKIE_EXT_COMMENT | COOKIE_XML_COMMENT)) == 0;
	...
	for (int I = 0; ; I++)
	{
		if (bRedefineBlock)
		{
			...
			if (bDecIndex)
				nPos--;
			if (dwCookie & (COOKIE_COMMENT | COOKIE_EXT_COMMENT | COOKIE_XML_COMMENT))
			{
		...
		if (dwCookie & COOKIE_XML_COMMENT)
		{
			if (I > 1 && pszChars[I] == '>' && pszChars[I - 1] == '-' && pszChars[I - 2] == '-' )
			{
				dwCookie &= ~COOKIE_XML_COMMENT;
				bRedefineBlock = TRUE;
			}
			continue;
		}
		...
		if (I > 2 && pszChars[I] == '-' && pszChars[I - 1] == '-' && pszChars[I - 2] == '!' && pszChars[I - 3] == '<')
		{
	...
	if (pszChars[nLength - 1] != '\\')
		dwCookie &= (COOKIE_EXT_COMMENT | COOKIE_XML_COMMENT);
	return dwCookie;
}
